...
tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2-medium", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("openai-community/gpt2-medium", trust_remote_code=True)
...
# json_file_path = "./FollowUp/gpt_m_senator_t01_followup.json"
...
for temp in [0.1]:
    ...
    for item in questions:
        ...
        # question = f"{item}" 
        # question = f"Directly answer this question in only one sentence: {question}"
        # question = f"{question} Directly answer this question."
    ...
    output_file_path = f"./Answers/gpt_m_primality_greedy_fs.json"
    # output_file_path = f"./Answers/gpt_m_arithmetic_greedy.json"
    # output_file_path = f"./Answers/gpt_m_senator_FU_t{str(temp).replace('.', '')}.json"
    # output_file_path = f"./Answers/gpt_m_primality_b{str(beam)}.json"
